...
class Database:
    ...
    def add_company_id_and_insert(self, table_name, df, required_columns, company_name_column='company_name'):
        ...
        print(company_ids)
        df['company_name'].replace({'현대자동차':'현대차','POSCO홀딩스':'포스코'},inplace=True)
    ...
    def select_data(self, query=None, table_name=None, start_date=None, end_date=None, company_ids=None, file_name=None):
        where_sql = []
        ...
        while True:
            ...
            with self.DB.cursor(pymysql.cursors.DictCursor) as cur:  # DictCursor를 사용
                ...
                if not column_names:  # 컬럼명 리스트가 비어있다면,
                    column_names = [i[0] for i in cur.description]  # cursor의 description 속성에서 컬럼명을 추출합니다.
